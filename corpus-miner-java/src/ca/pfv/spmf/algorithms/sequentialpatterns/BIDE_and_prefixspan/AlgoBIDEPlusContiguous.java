 ...
 * It features contiguous pseudo-projection, backscan-pruning, and forward
 * closure checks.
 */
public class AlgoBIDEPlusContiguous {
    ...
    private void expand(int[] prefix, List<Occurrence> occs) {
        ...
        boolean hasForwardExtension = false;
        int mySupport = getUniqueDocs(occs);

        for (Map.Entry<Integer, List<Occurrence>> entry : nextOccs.entrySet()) {
            int childSupport = getUniqueDocs(entry.getValue());

            // Forward Closure Check: if an extension has exactly the same support, the
            // current prefix is not closed
            if (childSupport == mySupport) {
                hasForwardExtension = true;
            }
        ...
        // If there's no forward extension with identical support, and we survived
        // BackScan, it's CLOSED.
        if (!hasForwardExtension) {
    ...
    public void printStatistics() {
        System.out.println("====== BIDE+ (Contiguous) STATISTICS ======");
        System.out.println(" Total time: " + (endTime - startTime) + " ms");
        System.out.println(" Mining time: " + (endTime - startTimeAfterLoadDatabase) + " ms");
        System.out.println(" Closed sequences count: " + patternCount);
